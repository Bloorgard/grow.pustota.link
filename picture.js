...
/* Доля двух самых крупных корзин гистограммы выше этой — плоская графика:
   ...
   Правило рассчитано на одно-двухтоновую графику: у неё две корзины
   собирают почти всё. Пёстрый логотип в четыре ровных цвета уедет в
   бинаризацию, и это поправимо кнопкой «два тона». */
...
export function analyze(img) {
  ...
  /* Стеной становится меньшинство: тёмный объект на светлом фоне или
     ...
     У одноцветной картинки меньшинства нет вовсе — инверсия там
     означала бы пустой холст без всякого объяснения. */
  return { binary, threshold: Math.round(cut / 255 * 100), invert: dark > opaque / 2 && dark < opaque };
}
